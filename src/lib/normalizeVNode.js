 ...
 */
export async function normalizeVNode(vNode) {
  ...
  if (typeof vNode.type === "function") {
    ...
    if (vNode.children?.length > 0) {
      props.children = await Promise.all(vNode.children.map(async (child) => await normalizeVNode(child)));
    }

    try {
      const result = vNode.type(props);
      if (isThenable(result)) {
        // async 컴포넌트의 Promise를 기다린 후 결과를 정규화
        const resolvedResult = await result;
        return await normalizeVNode(resolvedResult);
      }
  ...
  const normalizedChildren = Array.isArray(vNode.children)
    ? (await Promise.all(vNode.children.map(async (child) => await normalizeVNode(child)))).filter(
        (child) => child !== "" && child != null,
      )
    : vNode.children
      ? [await normalizeVNode(vNode.children)]
      : [];
